...
def mod_ax(ax):
    ...
    ax.axhline(0, ls="-",c="gray", lw=.7)
    ax.axvline(0, ls="-",c="gray", lw=.7)
...
def main():
    ...
    gs = fig.add_gridspec(nrows=1, ncols=5, width_ratios=[1,.1,1,.1,1])
    ...
    ax0.plot(agg_r2.lag, agg_r2["E_track"].sel(mask="Total"), c=colors[0], label="Tracked evap.")
    ax0.plot(agg_r2.lag, agg_r2["P"].sel(mask="Midwest"), c=colors[1], label="Midwest precip.")
    ax0.legend()
    ax0 = mod_ax(ax0)
